---
title: "Oakwood Memories"
type: link
era: 2000-present
description: "A 160-page compiled local history of Oakwood, Ohio, from the Oakwood Library Association."
source: "Oakwood Library Association, compiled by Jeanne Bennett Calvert, Quail Hollow Press, 2013"
rights: "Bibliographic reference only; this entry links to the book's catalog listing, not a digitized copy. Check availability/rights before reproducing its contents."
external_url: "https://books.google.com/books?id=SXIVngEACAAJ"
tags: [oakwood, local history, bibliography, further reading]
---

Catalog listing for a 2013 compiled local history of Oakwood, Ohio, produced
by the Oakwood Library Association. Community-compiled histories like this
one often preserve first-hand accounts and local memory that don't survive
in official county records.

If anyone has access to a physical or scanned copy, excerpts (with
permission) or a `document`-type entry summarizing specific accounts would
be a valuable addition.
